...
def test_g_vector():
    ...
    assert(np.allclose(kriging._g_vector(x,y, xi, yi, model), result, rtol=1e-4))
...
def test_ordinary():
    ...
    assert(np.allclose(kriging.ordinary(x,y,v, xi, yi, model), (result_v_est, result_v_var), rtol=1e-4))
...
def test_ordinary_mesh():
    ...
    result_v_est = 1.636390
    result_v_var = 0.420099
    print(kriging.ordinary_mesh(x,y,v, np.array([xi]), np.array([yi]), model))
    assert(np.allclose(kriging.ordinary(x,y,v, xi, yi, model), (result_v_est, result_v_var), rtol=1e-4))
